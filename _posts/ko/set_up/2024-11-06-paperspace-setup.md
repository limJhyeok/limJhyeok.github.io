...
`CORE`의 경우 Paperspace의 클라우드 인프라 서비스로, 가상 머신(VM), 스토리지, 네트워킹 등 기본적인 클라우드 컴퓨팅 자원을 제공합니다.
...
- RAM, CPU, GPU, 드라이브 등 컴퓨터 리소스 설정
    ...
    **How do you Intend to use this machine?** 텍스트 박스를 선택해서 GPU 서버 사용 목적을 입력하고 REQUEST MACHINE TYPE 버튼을 클릭하겠습니다.
    ...
    먼저 모델은 아까와 같이 GPU 스펙과 가격을 고려해서 정하시면 되고 다음으로 OS Template을 정해야 합니다.
    
    - **OS Template**
    
        OS는 현재 크게 Ubuntu, Window, CentOS가 존재합니다.
        
        저희는 현재 대중적으로 널리쓰이는 Ubuntu 22.04로 선택을 하겠습니다.
    
    - **Disk size**

        이후로 Disk size를 정해야하는데 학습하고자 하는 데이터 크기와 딥러닝 모델 파일 크기(e. g., .pt, .ggml 파일 등)를 고려해서 용량을 넉넉히 잡으시는게 좋습니다.
    ...
    - **Network**
        ...
        이후에는 Network 설정을 진행해야합니다.
        
        먼저 Region은 어느 지역에 있는 서버를 임대할 것인가 입니다. 일반적으로 사용자의 지역과 가까운 지역일수록 Network 연결이 보다 잘되겠죠? 이를 고려해서 선택하시면 됩니다.
        
        Public IP 설정은 인터넷을 사용할 것인가인데 인터넷 환경이 필요없는 개발환경이면 None을 선택하셔도 되지만 github 연결 또는 인터넷을 통해 데이터 다운로드 등에 어려움이 있다는 점을 고려해주세요.
    ...
    - **Authentication and Access**
        ...
        이제 `Authentication and Access` 설정에서 로컬 컴퓨터의 SSH 키를 등록하게 됩니다.
        ...
        - **SSH 키가 이미 있는 경우**: Public key를 복사하여 붙여넣고 이름을 지정하세요.
        - **SSH 키가 없는 경우**: 아래 가이드에 따라 새로 생성한 후 Public key를 등록하세요. 
    ...
    - **Snapshots**(참고: [Paperspace snapshot](https://docs.digitalocean.com/products/paperspace/machines/how-to/manage-snapshots/))

        <img src="/images/assets/2024-11-06-17-56-05.png" width="100%" />
        
        다음으로 `Snapshots` 설정인데 Snapshots은 현재 서버의 상태(환경, 데이터 등)를 백업하는 기능입니다. 서버가 예상치 못하게 내려가는 것과 같은 만일의 사태를 대비하기 위한 용도로 주요 사용합니다.
...
1. [**Remote - SSH**](https://marketplace.visualstudio.com/items?itemName=ms-vscode-remote.remote-ssh)  extension 설치
...
만약 Machine을 만든 후 로컬 컴퓨터의 SSH key가 변경이 되어서 다시 등록을 해야하거나 Disk 설정 등을 잘못 설정했을 경우 Machine을 다시 생성해야하는데 이럴 경우 처음부터 환경 설정을 다시 하고 필요한 데이터도 다시 다운로드 받아야하는 문제점이 있습니다. 이럴 경우에 대비해 `Paperspace CORE`에는 Template 기능(일종의 스냅샷)이 있어서 환경설정을 처음부터 구축할 필요없이 서버를 이관할 수 있는 기능을 제공하고 있습니다.
